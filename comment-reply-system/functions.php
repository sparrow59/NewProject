...
// connect to database
$db = mysqli_connect("localhost", "root", "lanesra1886", "baca_buku");
...
$post_query_result = mysqli_query($db, "SELECT * FROM books WHERE books_id='$books_id'");
$post = mysqli_fetch_assoc($post_query_result);

// Get all comments from database
$comments_query_result = mysqli_query($db, "SELECT * FROM comments WHERE post_id=" . $post['books_id'] . " ORDER BY created_at DESC");
...
{
	global $db;
	$result = mysqli_query($db, "SELECT fname FROM members WHERE id=" . $id . " LIMIT 1");
	// return the username
...
if (isset($_POST['comment_posted'])) {
	...
	$sql = "INSERT INTO comments (post_id, user_id, body, created_at, updated_at) VALUES ('$books_id', " . $user_id . ", '$comment_text', now(), null)";
	...
	if ($result) {
		...
		$comment_info = array(
			'comment' => $comment,
			'comments_count' => getCommentsCountByPostId($books_id)
		);
